...
@export_range(0.01, 1.0, 0.001) var circle_coverage : float = 0.66: set = _set_circle_coverage
@export_range(-1.578, 4.712, 0.001) var center_angle : float = -PI/2: set = _set_center_angle
@export var show_animation := true
@export_range(0.01, 1.0, 0.1) var animation_speed_factor : float = 0.2
@export_range(0, 10, 0.5) var outside_selection_factor : float = 3.0
@export_range(0.01, 2.0, 0.05) var icon_scale := 1.0: set = _set_icon_scale
...
func _set_radius(new_radius):
    radius = new_radius
    _calc_new_geometry()	
    #FIXME: Popup class doesn't inherit CanvasItem anymore
    #update()
...
func _create_subtree():
    ...
    if not get_node_or_null(TWEEN_NAME):
        ...
        add_child(tween)
        tween.loop_finished.connect(_on_Tween_tween_all_completed)
...
func _ready():		
    ...
    if not is_submenu:
        ...
        about_to_popup.connect(_about_to_show)
